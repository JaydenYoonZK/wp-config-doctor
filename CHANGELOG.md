...
- The brand link in the nav now carries Jayden's Chinese name, 袁正凯, rendered as a little seal beside the Latin name. The characters stack vertically on a chartreuse field with dark ink and an inner frame, the same color pairing the buttons use in both themes.

## [1.5.11] - 2026-08-16

### Fixed

- The film grain overlay was pinned to the screen, and on iOS Safari a full-bleed pinned layer makes the browser draw scrolled page content into the status bar strip around the clock. The grain now anchors to the page itself and scrolls with it, so the strip stays clean. It still covers the whole document and the texture looks exactly the same.

## [1.5.10] - 2026-08-16

### Fixed

- The green pill that hovered over the status bar on an iPhone is gone. It was the skip-to-content link all along, parked just above the viewport's top edge, exactly the strip iOS paints behind the clock. It waits off to the side now and still drops in on keyboard focus. Found and confirmed in the iOS Simulator, not guessed at.

### Changed

- The nav rides position fixed with safe-area padding, so it owns the zone under the status bar and a hard fling can never lag it a frame. The shields from 1.5.8 and 1.5.9 are removed: both tried to paint over the status-bar zone, and the Simulator settled that nothing inside a page draws there. The faint content showing through behind the status bar while scrolling is Safari's own edge-to-edge design, and apple.com shows it identically.

## [1.5.9] - 2026-08-16

### Fixed

- The shield above the nav now rides fixed to the viewport instead of sticky. Under fast scrolling Safari can lag a sticky element's repositioning by a frame, flashing content at exactly the wrong moment; a fixed element is pinned by the compositor and cannot lag.

## [1.5.8] - 2026-08-16

### Fixed

- On iPhones, scrolling could flash page content in the strip under the status bar, above the nav. The cover for that strip used to hang off the blurred bar itself, and iOS never drew it there; it now lives on its own shield element outside the bar.
